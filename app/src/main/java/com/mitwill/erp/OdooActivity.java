...
import com.mitwill.erp.core.account.ManageAccounts;
import com.mitwill.erp.core.account.OdooLogin;
import com.mitwill.erp.core.account.OdooUserAskPassword;
import com.mitwill.erp.core.account.OdooUserObjectUpdater;
...
import com.mitwill.erp.core.support.addons.fragment.IBaseFragment;
import com.mitwill.erp.core.support.drawer.ODrawerItem;
import com.mitwill.erp.core.support.sync.SyncUtils;
import com.mitwill.erp.core.utils.BitmapUtils;
import com.mitwill.erp.core.utils.OAlert;
import com.mitwill.erp.core.utils.OAppBarUtils;
import com.mitwill.erp.core.utils.OControls;
import com.mitwill.erp.core.utils.OFragmentUtils;
import com.mitwill.erp.core.utils.OResource;
import com.mitwill.erp.core.utils.drawer.DrawerUtils;
import com.mitwill.erp.core.utils.sys.IOnActivityResultListener;
import com.mitwill.erp.core.utils.sys.IOnBackPressListener;
...
public class OdooActivity extends OdooCompatActivity {
    ...
    public IOnDrawerItemClickListener mOnDrawerItemClickListener = null;
    ...
    private void callImageUploadService() {
        ArrayList<Integer> list= getImageForOrderList();
        for (int listOrder:list) {
            Bundle dataimage = new Bundle();
            dataimage.putInt("order_id", listOrder);
            Intent imageIntent= new Intent(this, ImageUploadService.class);
            imageIntent.putExtras(dataimage);
            this.startService(imageIntent);
            Log.d(TAG, "callImageUploadService: "+"call from odooActivity");
        }
    }

    private ArrayList<Integer> getImageForOrderList() {
        ...
        List<ODataRow> imgesListData = attachedImages.select(null,
                "is_uploaded = ? ", new String[]{ "false"});
        int prev_id=0;
        ArrayList<Integer> list =new ArrayList<>();
        for (ODataRow dataRow : imgesListData) {
            if(prev_id != dataRow.getInt(AttachedImages.ID)){
                prev_id = dataRow.getInt(AttachedImages.ID);
                list.add(dataRow.getInt(AttachedImages.ID));
            }
        }
        return list;
    }
    ...
    private void setupDrawer() {
        ...
        tvVersion = mDrawerLayout.findViewById(R.id.tvVersion);
    ...
    public View.OnClickListener drawerItemClick = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if (isButtonClicked){
                isButtonClicked = false;
                int index = mDrawerItemContainer.indexOfChild(v);
                if (mDrawerSelectedIndex != index || mDrawerSelectedIndex == 1) {
                    ...
                    if (mOnDrawerItemClickListener != null && item.getTitle().equalsIgnoreCase("My Orders")) {
                        mOnDrawerItemClickListener.onDrawerItemClicked(mDrawerSelectedIndex);
                    }
                }
            }
            closeDrawer();
    ...
    private void accountListDefaultItems() {
        // Adding add account
        View view = generateView(OResource.string(this, R.string.label_drawer_account_manage_accounts),
