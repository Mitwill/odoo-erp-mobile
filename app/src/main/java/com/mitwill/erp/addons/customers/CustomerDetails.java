...
import com.mitwill.erp.addons.customers.utils.ShareUtil;
import com.mitwill.erp.addons.saleorder.models.SaleOrder;
import com.mitwill.erp.addons.ir.feature.OFileManager;
...
import com.mitwill.erp.core.utils.OStringColorUtil;
...
public class CustomerDetails extends OdooCompatActivity
    ...
    private void createOrderForUser() {
        ...
        values.put( "partner_id", record.getInt( OColumn.ROW_ID ) );
        values.put( "incoterm", 1 );
        values.put( "name","/" );
        final int row_id = saleOrder.insert( values );
        //request immedicate sync
//        saleOrder.sync().requestSync( SaleOrder.AUTHORITY );
        if ( row_id != OModel.INVALID_ROW_ID ) {
            Toast.makeText( this, getResources().getString(R.string.label_order_created), Toast.LENGTH_LONG );
            finish();
        }
    }
