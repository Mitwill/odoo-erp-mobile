...
public class OSyncDataUtils {
    ...
    public void handleResult(OModel model, OUser user, SyncResult result, OdooResult response, Boolean createRelRecord, SyncServiceCallBack handler) {
        mModel = model;
        mUser = user;
        mResult = result;
        ...
        handleResult(updateInLocal, handler);
    }
    ...
    private void handleResult(List<OdooRecord> records, SyncServiceCallBack callbackHandler) {
        try {
            recordsId.clear();
            ...
            callbackHandler.onSyncCompleted();
