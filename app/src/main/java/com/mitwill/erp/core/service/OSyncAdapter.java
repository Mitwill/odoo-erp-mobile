 ...
 */
package com.mitwill.erp.core.service;
...
import com.mitwill.erp.addons.ir.IrModel;
...
import com.mitwill.erp.core.utils.ODateUtils;
import com.mitwill.erp.core.utils.OPreferenceManager;
import com.mitwill.erp.core.utils.OResource;
import com.mitwill.erp.core.utils.OdooRecordUtils;
import com.mitwill.erp.core.utils.logger.OLog;
...
public class OSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void syncData( OModel model, OUser user, ODomain domain_filter,
        ...
        Log.v( TAG, "Sync for ("+model.getModelName()+") Staed at "+ODateUtils.getDate() );
        model.onSyncStarted();
        try {
            ...
            if ( checkForWriteCreateDate ) {
                ...
                if ( model.checkForCreateDate() && checkForDataLimit ) {
                    ...
                    long numberOfDays = DateUtils.numberOfDaysSinceDate( "01-01-2016" );
                    int data_limit = preferenceManager.getInt( "sync_data_limit", (int)numberOfDays );
                ...
                if ( model.checkForWriteDate() && !model.isEmptyTable() && createRelationRecords ) {
                    ...
                    if (model.getModelName().equals("product.product")){
                        OModel productTemplate=new OModel(getContext(), "product.template",user);
                        String last_sync_date_template = productTemplate.getLastSyncDateTime();
                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

                        if(sdf.parse(last_sync_date).after(sdf.parse(last_sync_date_template))){
                            last_sync_date=last_sync_date_template;
                        }
                    }
                    if ( last_sync_date != null ) {
                        domain.add( "write_date", ">", last_sync_date );
                        Log.d("syncData:date ",last_sync_date);
                    }
                }
            }

            Log.d(TAG, "syncData: Domain"+domain.get());
            ...
            if ( response.containsKey( "error" ) ) {
                ...
                if ( pref.getBoolean( Constants.PreferenceConstants.DEVELOPER_MODE, false ) ) {
            ...
            if ( model.allowCreateRecordOnServer() ) {
                ...
                Log.d( TAG, "#######syncData:allowCreateRecordOnServer "+model.getModelName() );
            } else {
                Log.d( TAG, "#######syncData:allowCreateRecordOnServer  NOT called "+model.getModelName() );
            }
            ...
            if ( createRelationRecords ) {
                ...
                OModel productTemplate=new OModel(getContext(), "product.template",user);
                irModel = new IrModel( mContext, user );
                irModel.setLastSyncDateTimeToNow( productTemplate );
            }
    ...
    public static OdooFields getFields( OModel model ) {
    ...
    public static OdooFields getOrderFields( OModel model ) {
        OdooFields fields = new OdooFields();
        List<String> names = new ArrayList<>();
        for ( OColumn column : model.getColumns( false ) ) {
            if (column.getSyncColumn().equals("designCode") || column.getSyncColumn().equals("productName") )
                continue;
            names.add( column.getSyncColumn() );

            Log.d(TAG, "doInBackground:" +  column.getSyncColumn());
        }
        fields.addAll( names.toArray( new String[names.size()] ) );
        return fields;
    }
