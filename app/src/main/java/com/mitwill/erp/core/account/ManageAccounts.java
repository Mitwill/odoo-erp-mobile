        ...
        switch (v.getId()) {
            ...
            case R.id.btnRemoveAccount:
                ...
                builder.setPositiveButton(R.string.label_delete, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        OUser user = (OUser) v.getTag();
                        new AccountDeleteTask(user.getAndroidName()).execute(user);
//                        setResult(RESULT_OK);
//                        finish();
                    }
                });
    ...
    private class AccountDeleteTask extends AsyncTask<OUser, Void, Boolean> {
        ...
        Boolean isActiveUserDelete = false;
        String androidName;

        public AccountDeleteTask(String androidName) {
        ...
        protected Boolean doInBackground(OUser... odooUsers) {
            ...
            try {
                for (OUser odooUser : odooUsers) {
                    if (odooUser.getAndroidName().equalsIgnoreCase(OdooAccountManager.getActiveUser(ManageAccounts.this).getAndroidName())) {
                        isActiveUserDelete = true;
                        return OdooAccountManager.removeAccount(ManageAccounts.this, odooUser.getAndroidName());
                    } else if (odooUser.getAndroidName().equalsIgnoreCase(androidName)) {
                        return OdooAccountManager.removeAccount(ManageAccounts.this, odooUser.getAndroidName());
                    }
                }
            }catch (Exception e){
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                ...
                hideProgressDialog();
                if (accounts.size() > 0) {
                    if (isActiveUserDelete) {
                        ArrayList<OUser> data = new ArrayList<>(OdooAccountManager.getAllAccounts(ManageAccounts.this));
                        if (data.size() > 0) {
                            OUser updatedUser = new OUser();
                            data.get(data.size() - 1).setIsActive(true);
                            updatedUser.setFromBundle(data.get(data.size() - 1).getAsBundle());
                            OdooAccountManager.updateUserData(ManageAccounts.this, data.get(data.size() - 1), updatedUser);
                        }
                    }
                    setResult(RESULT_OK);
                    finish();
                    ...
                    //redirect to login
                    Intent i = new Intent(ManageAccounts.this, OdooLogin.class);
    ...
    private void setProgressDialog() {
        progressDialog = new ProgressDialog(ManageAccounts.this);
